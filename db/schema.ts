...
export const wordStatusEnum = pgEnum('word_status', ['unknown', 'learning', 'known']);
...
export const userBookProgress = pgTable(
  ...
  (table) => ({
    userBookUnique: uniqueIndex('ubp_user_book_unique').on(table.userId, table.bookId),
    userUpdatedIdx: index('idx_ubp_user_updated_at').on(table.userId, table.updatedAt.desc()),
    bookIdx: index('idx_ubp_book').on(table.bookId),
    lastIndexCheck: check('ubp_last_index_nonneg', sql`${table.lastIndex} >= 0`),
    userFk: foreignKey({
      name: 'ubp_user_fk',
...
export const learningSessions = pgTable(
  ...
  (table) => ({
    ...
    userStartedIdx: index('idx_ls_user_started').on(table.userId, table.startedAt.desc()),
  }),
);
...
export const events = pgTable(
  ...
  (table) => ({
    ...
    userTimeIdx: index('idx_events_user_time').on(table.userId, table.createdAt.desc()),
    typeTimeIdx: index('idx_events_type_time').on(table.eventType, table.createdAt.desc()),
  }),
);
